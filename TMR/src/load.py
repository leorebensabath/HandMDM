...
def load_model_from_cfg(cfg, ckpt_name="last", device="cpu", eval_mode=True, return_epoch=False, reload_model=False):
    import torch
    ...
    if return_epoch:
        ckpt_path = os.path.join(run_dir, "logs", "checkpoints", f"{ckpt_name}.ckpt")
        if not torch.cuda.is_available():
            epoch = torch.load(ckpt_path, map_location=torch.device('cpu'))["epoch"]
        else:
            epoch = torch.load(ckpt_path)["epoch"]

    if not os.path.exists(pt_path) or reload_model:
