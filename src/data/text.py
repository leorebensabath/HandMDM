...
def save_text_embeddings_augmented(path, augmentation_path, modelname, device="cuda"):
    model = TextToEmb(modelname, device=device)
    annotations = load_annotations(path)

    with open(augmentation_path) as f:
        augmentation_dict = json.load(f)

    path = os.path.join(path, TextEmbeddings.name)
    ptpath = os.path.join(path, f"{modelname}_augmented.npy")
    slicepath = os.path.join(path, f"{modelname}_slice_augmented.npy")
    jsonpath = os.path.join(path, f"{modelname}_index_augmented.json")
    ...
    for dico in annotations.values():
        for lst in dico["annotations"]:
            all_texts.extend(augmentation_dict[lst["text"]])
