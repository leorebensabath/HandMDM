...
class TMR(TEMOS):
    ...
    def __init__(
        ...
        self.freeze_motion_encoder = freeze_motion_encoder
        self.freeze_motion_decoder = freeze_motion_decoder
        
        if self.freeze_motion_encoder:
            for i, param in enumerate(self.motion_encoder.parameters()):
                param.requires_grad = False
        if self.freeze_motion_decoder:
            for i, param in enumerate(self.motion_decoder.parameters()):
                param.requires_grad = False
    ...
    def compute_loss(self, batch: Dict, return_all=False) -> Dict:
        ...
        # fmt: off
        if self.text_branch:
            t_reconstruction_loss = self.reconstruction_loss_fn(t_motions, ref_motions)
        else:
            t_reconstruction_loss = 0
        if not self.freeze_motion_encoder:
            m_reconstruction_loss = self.reconstruction_loss_fn(m_motions, ref_motions)
        else:
            m_reconstruction_loss = 0
        losses["recons"] = (
            + t_reconstruction_loss # text -> motion
            + m_reconstruction_loss # motion -> motion
        )
        # fmt: on
        ...
        if self.vae:
            ...
            if self.text_branch:
                mt_kl = self.kl_loss_fn(m_dists, t_dists)
                tm_kl = self.kl_loss_fn(t_dists, m_dists) 
                t_kl = self.kl_loss_fn(t_dists, ref_dists)
            else:
                mt_kl = 0
                tm_kl = 0
                t_kl = 0
            if not self.freeze_motion_encoder:
                m_kl = self.kl_loss_fn(m_dists, ref_dists)
            else:
                m_kl = 0

            losses["kl"] = (
                mt_kl  # motion_to_text
                + tm_kl  # text_to_motion
                + m_kl  # motion
                + t_kl  # text
            )
