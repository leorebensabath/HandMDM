...
class Llama3_wrapper(nn.Module):
    ...
    def forward(self, texts: List[str], device=None) -> Dict:
        device = device if device is not None else self.device
        return self.model.encode(texts).to(device)


class OneHotEncoding_wrapper(nn.Module):
    def __init__(self, device: str = "cpu"):
        super().__init__()
        self.device = device

        with open("/linkhome/rech/genlgm01/ujv31bi/bsltrain-master/leore_scripts/data/one_hot_encoding.pickle", "rb") as f:
            gloss_onehot_encoding = pickle.load(f)
    
        self.gloss_onehot_encoding = {key.lower(): val for key, val in gloss_onehot_encoding.items()}
        self.gloss_onehot_encoding[""] = np.zeros(236)

    @torch.no_grad()
    def forward(self, texts: List[str], device=None) -> Dict:
        device = device if device is not None else self.device
        onehot = [torch.tensor(self.gloss_onehot_encoding[text]) for text in texts]
        return torch.stack(onehot).to(device)
...
def TextToEmb(modelpath: str, mean_pooling: bool = False, device: str = "cpu"):
    ...
    # hugging face
    elif modelpath == "llama3":
        return Llama3_wrapper(device=device)
    
    elif modelpath == "onehot":
        return OneHotEncoding_wrapper(device=device)
