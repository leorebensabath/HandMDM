...
def monitor_resources(interval=1):
    print(f"{'Time':<10} {'CPU (%)':<10} {'Memory (MB)':<15} {'Available Memory (MB)':<20}")
    # CPU usage
    ...
    # Print the results
    print(f"{time.strftime('%H:%M:%S'):<10} {cpu_percent:<10} {used_memory_mb:<15.2f} {available_memory_mb:<20.2f}")
...
class GaussianDiffusion(DiffuserBase):
    ...
    def __init__(
        ...
        #if self.motion_normalizer is None:
        #    self.motion_normalizer = lambda x: x
        print("self.motion_normalizer.disable: ", self.motion_normalizer.disable)
    ...
    def on_train_epoch_end(self):
        ...
        self.losses = []
        self.log_dict(dico, rank_zero_only=True)
